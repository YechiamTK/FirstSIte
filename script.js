...
$(document).ready(function () {
    ReactDOM.render(React.createElement(TopNavbar, null), document.getElementById('top-navbar'));
    for (var i = 0; i < tweets.length; i++) {
        newTweet(tweets[i], allTweets);
    }
    ReactDOM.render(React.createElement(MainRow, { tweets: allTweets }), document.getElementById('main-row'));
...
function newTweet(tweet_info, allTweets) {
    ...
    var newTweet = new Tweet(tweet_info.username, tweet_info.username, tweet_info.tweet);
...
function newComment(elem) {
    ...
    var message = $(elem).parents(".modal-footer").siblings(".modal-body").find("#tweet-text").val();
    var newTweet = new Tweet(currUser, currUser, message);
    var newId = newTweet.getId();
    allTweets.push(newTweet);

    //push the new Tweet as a comment inside the previous Tweet
    var prevId = $(elem).parents(".modal-footer").siblings(".modal-body").find(".card-header").find(".tweet-id").text();
    var prevTweetPlace = getTweetPlaceById(prevId, allTweets);
    var newTweetPlace = getTweetPlaceById(newId, allTweets); //perhaps I can bypass this by passing the original Tweet object, but idk how js will behave
...
function showHideComments(elem, showFlag) {
    if (showFlag == true) {
        var parentId = $(elem).parents(".card-footer").siblings(".card-header").find(".tweet-id").text();
        var prevTweetPlace = getTweetPlaceById(parentId, allTweets);
        var prevTweet = allTweets[prevTweetPlace];
        var tweetsToComment = prevTweet.getComments();

        for (var i = 0; i < tweetsToComment.length; i++) {
            var newCard = $(elem).parents(".modal-content").find("ul li:first").clone();
            var currTweet = tweetsToComment[i];
        ...
        var cardsList = $(elem).parents(".modal-content").find("ul");
...
function postTweet() {
    ...
    var tweet_json = { username: currUser, tweet: text };
    $("#tweet-text").val('');
