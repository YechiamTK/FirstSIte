...
document.addEventListener("DOMContentLoaded", function () {
  ...
  ReactDOM.render( /*#__PURE__*/React.createElement(ReactRedux.Provider, {
    ...
    tweets: allTweets
  })), rootElem); //ReactDOM.render(<TopNavbar />, document.getElementById('top-navbar'));
...
function newTweet(tweet_info, allTweets) {
  ...
  let counter = add(); //create a new Tweet object with the tweet's info and push it to the tweets' db

  let newTweet = new Tweet(tweet_info.username, tweet_info.username, tweet_info.tweet);
  allTweets.push(newTweet); //let card_class = document.createElement("div");
...
function newComment(elem) {
  ...
  allTweets.push(newTweet); //push the new Tweet as a comment inside the previous Tweet
...
function showHideComments(elem, showFlag) {
  if (showFlag == true) {
    ...
    for (let i = 0; i < tweetsToComment.length; i++) {
      ...
      $(newCard).attr({
        //clear the "clone" attributes from our new card
        'class': 'card',
        'style': ''
      });
  ...
  $(elem).attr({
    'onclick': 'showHideComments(this, ' + !showFlag + ')'
  });
}
/*
